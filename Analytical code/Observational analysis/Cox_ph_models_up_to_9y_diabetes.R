...
load(file = "path/Data/Data_models/data_models_diabetes_up_to_9y_exact_matching_w_mortality.RData")
...
mfull_diabetes %>%
 ...
 mutate(analysis = "up to 9 years of age") %>%
 write.csv(file = "path/Results/Cox_ph_models_ex_matching_up_to_9y_w_mortality.csv",
           row.names = FALSE)
...
reg_plot <- mfull_diabetes %>%
 ...
 {ggplot(data = ., aes(x = estimate, y = outcome_names, xmin = conf.low, xmax = conf.high)) +
   ...
   scale_x_log10(limits = c(0.3, 9), 
                 breaks = c(0.3, 0.5, 0.7, 1, 1.5, 2, 3, 5, 7)) +
   xlab("aHR (95% CI)") +
...
ggsave(file = "path/Results/HR_plot_up_to_9y.eps",
       device = "eps",
       width = 40,
       height = 20,
       units = "cm",
