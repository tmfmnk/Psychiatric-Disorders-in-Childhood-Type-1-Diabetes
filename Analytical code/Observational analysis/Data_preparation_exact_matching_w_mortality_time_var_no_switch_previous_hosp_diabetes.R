...
#Keeping hospitalizations between 1997 and 2007
#Keeping individuals aged 0-14
#Excluding individuals with residence outside of Czechia

hospitalizations_exp_1997_2007 <- hospitalizations_1994_2017 %>%
 filter(year(ymd(DATPRI)) >= 1997 & year(ymd(DATUKO)) <= 2007) %>%
 filter(VEK <= 14) %>%
...
set.seed(123)
first_hospitalizations_exp_1997_2007 <- hospitalizations_exp_1997_2007 %>%
...
rm(hospitalizations_exp_1997_2007)

#Computing the number of hospitalizations in the past 3 years

first_hospitalizations_exp_1997_2007 <- first_hospitalizations_exp_1997_2007 %>%
 left_join(first_hospitalizations_exp_1997_2007 %>%
            transmute(RODCIS2,
                      historic_comorbidity_followup_start = ymd(DATPRI) %m-% years(3),
            ...
            mutate(past_hospitalizations = int_overlaps(interval(comorbid_hospitalization_start, comorbid_hospitalization_end),
                                                        interval(historic_comorbidity_followup_start, historic_comorbidity_followup_end))) %>%
            group_by(RODCIS2) %>%
            summarise(past_hospitalizations = sum(past_hospitalizations) - 1) %>%
            ungroup(),
           by = c("RODCIS2" = "RODCIS2"))
...
hospitalizations_unexp_1997_2007 <- hospitalizations_1994_2017  %>%
 filter(year(ymd(DATPRI)) >= 1997 & year(ymd(DATUKO)) <= 2007) %>%
 filter(VEK <= 14) %>%
 mutate(ZDG = trimws(ZDG),
        DDG2 = trimws(DDG2),
        ...
        DDG4 = trimws(DDG4),
        DDG5 = trimws(DDG5)) %>%
 mutate(cond = DATPRI < first_hospitalizations_exp_1997_2007$DATPRI[match(RODCIS2, first_hospitalizations_exp_1997_2007$RODCIS2)]) %>%
...
set.seed(123)
hospitalizations_unexp_1997_2007 <- hospitalizations_unexp_1997_2007 %>%
 group_by(RODCIS2, DATPRI, DATUKO) %>%
 filter(row_number() == sample(1:n(), 1)) %>%
 ungroup()
...
hospitalizations_unexp_1997_2007 <- hospitalizations_unexp_1997_2007 %>%
  ...
  left_join(hospitalizations_unexp_1997_2007 %>%
              transmute(RODCIS2,
                        rowid = row_number(),
                        historic_comorbidity_followup_start = ymd(DATPRI) %m-% years(3),
                        historic_comorbidity_followup_end = ymd(DATPRI)) %>%
...
matched_pairs <- first_hospitalizations_exp_1997_2007 %>%
 transmute(RODCIS2_exposed = RODCIS2,
           VEK,
           POHL,
           month_discharge = month(ymd(DATUKO)),
           year_discharge = year(ymd(DATUKO)),
           past_hospitalizations = case_when(past_hospitalizations >= 6 & past_hospitalizations <= 7 ~ "6-7",
                                             past_hospitalizations >= 8 & past_hospitalizations <= 9 ~ "8-9",
                                             past_hospitalizations >= 10 & past_hospitalizations <= 15 ~ "10-15",
                                             past_hospitalizations > 15 & past_hospitalizations <= 20 ~ "16-20",
                                             past_hospitalizations > 20 & past_hospitalizations <= 30 ~ "21-30",
                                             past_hospitalizations > 30 & past_hospitalizations <= 40 ~ "31-40",
                                             past_hospitalizations > 40 & past_hospitalizations <= 50 ~ "41-50",
                                             past_hospitalizations > 50 ~ "51+",
                                             TRUE ~ as.character(past_hospitalizations))) %>%
 inner_join(hospitalizations_unexp_1997_2007 %>%
            ...
            by = c("VEK" = "VEK",
                   ...
                   "past_hospitalizations" = "past_hospitalizations")) %>%
 filter(RODCIS2_exposed != RODCIS2) %>%
 filter(!grepl("^99", NBYDL)) %>%
 filter(!(grepl("^F1|^F2|^F3|^F4|^F5|^F6", ZDG) | grepl("^F1|^F2|^F3|^F4|^F5|^F6", DDG2) | grepl("^F1|^F2|^F3|^F4|^F5|^F6", DDG3) | grepl("^F1|^F2|^F3|^F4|^F5|^F6", DDG4) | grepl("^F1|^F2|^F3|^F4|^F5|^F6", DDG5))) %>%
 add_count(RODCIS2_exposed) %>%
 arrange(n, RODCIS2_exposed) %>%
 select(-n)
...
recursive_sampling <- function(data, n) {
  ...
  ux <- data$RODCIS2
  exl <- split(1:nrow(data), data$RODCIS2_exposed)
  for (ii in exl) {
...
set.seed(123)
matched_sampled_pairs <- recursive_sampling(matched_pairs, 5L)

#Sanity check
#Are all exposed individuals matched?

all(first_hospitalizations_exp_1997_2007$RODCIS2 %in% matched_sampled_pairs$RODCIS2_exposed)

#Number of unmatched individuals

sum(!first_hospitalizations_exp_1997_2007$RODCIS2 %in% matched_sampled_pairs$RODCIS2_exposed)

#Combine datasets

cohorts_baseline <- first_hospitalizations_exp_1997_2007 %>%
...
save(data_models, 
     file = "path/Data/Data_models/data_models_exact_matching_w_mortality_previous_hosp.RData")
