...
load(file = "path/Data/Data_models/data_models_exact_matching_w_mortality_previous_hosp.RData")
...
mfull_diabetes <- map2_dfr(.x = outcomes[ind],
                           ...
                           ~ tidy(coxph(as.formula(paste("Surv(", 
                                                         ...
                                                         ")",
                                                         "~ group + VEK + as.factor(POHL) + month_discharge + year_discharge + as.factor(past_hospitalizations) + strata(RODCIS2_exposed)")),
                                        data = .y),
                                  conf.int = TRUE,
                                  exponentiate = TRUE) %>%
                             mutate(outcome = .x) %>%
...
mfull_diabetes %>%
 ...
 mutate(analysis = "previous hospitalizations") %>%
 write.csv(file = "path/Results/Cox_ph_models_ex_matching_w_mortality_previous_hosp.csv",
           row.names = FALSE)
...
reg_plot <- mfull_diabetes %>%
 ...
 {ggplot(data = ., aes(x = estimate, y = outcome_names, xmin = conf.low, xmax = conf.high)) +
   ...
   scale_x_log10(limits = c(0.2, 11), 
                 breaks = c(0.2, 0.5, 0.7, 1, 1.25, 1.5, 2, 3, 5, 7, 11)) +
   xlab("aHR (95% CI)") +
...
ggsave(file = "path/Results/HR_plot_previous_hosp.eps",
       device = "eps",
       width = 40,
       height = 20,
       units = "cm",
