...
#Keeping hospitalizations between 1994 and 2007
#Keeping individuals aged 0-14
#Keeping records related to asthma
...
hospitalizations_unexp_1994_2007 <- hospitalizations_1994_2017  %>%
 ...
 filter(grepl("^J45", ZDG) | grepl("^J45", DDG2) | grepl("^J45", DDG3) | grepl("^J45", DDG4) | grepl("^J45", DDG5)) %>%
 select(-cond)
...
matched_pairs <- first_hospitalizations_exp_1994_2007 %>%
...
set.seed(123)
matched_sampled_pairs <- recursive_sampling(matched_pairs, 3L)

#Sanity check
#Are all exposed individuals matched?

all(first_hospitalizations_exp_1994_2007$RODCIS2 %in% matched_sampled_pairs$RODCIS2_exposed)

#Number of unmatched individuals

sum(!first_hospitalizations_exp_1994_2007$RODCIS2 %in% matched_sampled_pairs$RODCIS2_exposed)

#Combine datasets

cohorts_baseline <- first_hospitalizations_exp_1994_2007 %>%
 filter(RODCIS2 %in% matched_sampled_pairs$RODCIS2_exposed) %>%
...
save(data_models, 
     file = "path/Data/Data_models/data_models_exact_matching_w_mortality_asthma_comp.RData")